...
public class TransparentTransmission {
	...
	public TransparentTransmission() {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case MSG_DATA:
					if (listener != null) {
						listener.onReceive(msg.getData().getByteArray(KEY_BUFFER), msg.getData().getInt(KEY_LENGTH));
					}
					break;
				case MSG_TCPCLIENT:
					if (listener != null) {
    ...
    public TransparentTransmission(NetworkProtocol protocol) {
        this();
	...
	public boolean open(int timeout) {
		...
		boolean success = transmission.open(timeout);
